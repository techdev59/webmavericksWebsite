    ...
    <section className="portfolio section-padding">
      <div className="container">
        <div className="row justify-content-center">
          {data.map((item) => (
            <div
              className={`col-lg-${item.style.width} ${
                item.style.align ? item.style.align : ""
              }`}
              key={item.id}
            >
              <div
                className={`item md-mb80 ${
                  item.style.isFullWidth ? "full-width" : ""
                } ${item.style.mt ? "mt-" + item.style.mt : ""} ${
                  item.style.mb ? "mb-" + item.style.mb : ""
                }`}
              >
                {item.style.innerCol ? (
                  <div className="row justify-content-center">
